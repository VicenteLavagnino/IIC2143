class ExchangesController < ApplicationController
    def lobby_exchange
        ...
        @exchanges_done_finalized = ExchangeDone.where(state: 'Completada').where("seller_id = ? OR bidder_id = ?", current_user.id, current_user.id)
        @exchanges_done_failed = ExchangeDone.where(state: 'Fallida').where("seller_id = ? OR bidder_id = ?", current_user.id, current_user.id)
    end
    def confirmation
